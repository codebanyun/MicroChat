...
DEFINE_string(forward_service, "/service/forward_service", "转发管理子服务名称");
...
int main(int argc, char *argv[]) {
    ...
    MicroChat::GatewayServerFactory factory;
    if (!factory.create_redis_client(FLAGS_redis_host, FLAGS_redis_port, FLAGS_redis_db, FLAGS_redis_keep_alive)) {
    ...
    if (!factory.create_etcd_finder_clients(
        ...
        FLAGS_forward_service)) {
        LOG_ERROR("Failed to create Etcd finder client");
        return -1;
    }
    factory.server_config(FLAGS_websocket_listen_port, FLAGS_http_listen_port);
    auto gateway_server = factory.build();
    if (!gateway_server) {
        LOG_ERROR("Failed to build GatewayServer");
        return -1;
    }
