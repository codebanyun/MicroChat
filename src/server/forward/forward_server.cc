...
DEFINE_string(instance_name, "/forward_service/instance", "当前实例名称");
...
int main(int argc , char* argv[]) {
    ...
    MicroChat::ForwardServerFactory factory;
    ...
    auto server = factory.build();
    if (!server) {
        LOG_ERROR("Failed to build Forward server");
        return -1;
    }
    server->start();
